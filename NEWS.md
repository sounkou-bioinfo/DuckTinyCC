...
- **demo (jgd graphics device)**: added `demo/jgd_ducktinycc_demo.sh`, `demo/jgd_ducktinycc_udf.c`, and `demo/jgd_capture_server.py`, demonstrating a jgd R graphics device opened from a DuckDB SQL UDF compiled by DuckTinyCC. The demo starts a minimal jgd JSONL capture server, calls an embedded-R UDF from DuckDB, and prints captured frame/message counts plus drawing-operation histograms.

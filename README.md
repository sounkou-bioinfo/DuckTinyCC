...
`tcc_module(...)` assumes its callers are allowed to run native code. A
caller who can submit arbitrary SQL to it can compile C in the DuckDB
process. `-nostdlib` makes linking and deployment deterministic; it does
not isolate generated code. Explicit libraries, process APIs, callbacks,
assembly, syscalls, and invalid pointers behave as they do in any
in-process C program.

## Documentation

The rendered README is the landing page at
<https://sounkou-bioinfo.github.io/DuckTinyCC/>. Its executable SQL,
CLI, and R chunks are documentation **and** integration checks:
`make rdm` rebuilds the extension, runs them, and refreshes `README.md`.
The site also provides the [SQL
reference](https://sounkou-bioinfo.github.io/DuckTinyCC/reference.html),
[internals and
ownership](https://sounkou-bioinfo.github.io/DuckTinyCC/internals.html),
and [development
gates](https://sounkou-bioinfo.github.io/DuckTinyCC/development.html).

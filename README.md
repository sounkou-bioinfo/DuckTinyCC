...
``` bash
if [ -f build/release/ducktinycc.duckdb_extension ]; then
duckdb -unsigned <<'SQL'
...
SQL
  ...
  echo "build/release/ducktinycc.duckdb_extension not found; run make release first."
fi
```
...
SELECT * FROM tcc_module(mode := 'tcc_new_state');
SELECT * FROM tcc_module(mode := 'add_source', source := 'long long times2(long long x){ return x * 2; }');
SELECT * FROM tcc_module(mode := 'tinycc_bind', symbol := 'times2', sql_name := 'times2');
SELECT * FROM tcc_module(mode := 'compile', return_type := 'i64', arg_types := ['i64']);
...
if ! command -v R >/dev/null 2>&1; then
  echo "R not found; skipping embedded-R demo."
elif [ ! -f build/release/ducktinycc.duckdb_extension ]; then
  echo "build/release/ducktinycc.duckdb_extension not found; run make release first."
else

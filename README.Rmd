...
`DuckTinyCC` is a `DuckDB` `C` extension for in-process JIT compiled C UDFs allowing `C` scripting via `TinyCC`.
...
- Host-side marshalling scratch buffers (`arg_ptrs`, column pointer arrays) are allocated with `duckdb_malloc` and freed before callback return.

...
This example compiles inline source and registers immediately without staged state reuse.
...
This example loads a manual module init symbol that registers a SQL function through host helpers.
...
This chunk shows the current registry and reset behavior.
Loaded/codegen modules are pinned for safety, so `unregister` returns `E_UNSAFE_UNLOAD`.

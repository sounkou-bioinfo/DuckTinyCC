...
- `ptr`/`pointer` use explicit pointer ABI in generated wrappers and map to SQL `UBIGINT`
...
- current `struct<...>` and `map<...>` members support fixed-width scalar families only (`bool/i8/u8/i16/u16/i32/u32/i64/u64/ptr/f32/f64/uuid/date/time/timestamp/interval/decimal`)
- `union<name:type;...>` tokens are parsed but SQL runtime marshalling is not enabled yet (currently returns `E_BAD_SIGNATURE`)
...
- DuckDB nested types: `LIST`, `ARRAY`, `STRUCT`, and `MAP` are first-class SQL argument/return tokens with C descriptors; `UNION` tokens are parsed but runtime-marshalling is still blocked on C API vector access; nested/non-fixed-width members remain future work.

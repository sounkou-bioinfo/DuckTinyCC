...
static void tcc_exec_cleanup(tcc_exec_call_t *call) {
	...
	if (call->row_varchar_allocations) {
		for (row = 0; row < call->row_varchar_alloc_count; row++) {
			if (call->row_varchar_allocations[row]) {
				duckdb_free(call->row_varchar_allocations[row]);
			}
		}
		duckdb_free(call->row_varchar_allocations);
	}
	if (call->batch_varchar_owned && call->sig) {
		for (col = 0; col < call->sig->arg_count; col++) {
			if (!call->batch_varchar_owned[col]) {
				continue;
			}
			for (row = 0; row < call->n; row++) {
				if (call->batch_varchar_owned[col][row]) {
					duckdb_free(call->batch_varchar_owned[col][row]);
				}
			}
			duckdb_free(call->batch_varchar_owned[col]);
		}
	}
	if (call->batch_varchar_columns && call->sig) {
		for (col = 0; col < call->sig->arg_count; col++) {
			if (call->batch_varchar_columns[col]) {
				duckdb_free((void *)call->batch_varchar_columns[col]);
			}
		}
	}
	if (call->batch_blob_columns && call->sig) {
		for (col = 0; col < call->sig->arg_count; col++) {
			if (call->batch_blob_columns[col]) {
				duckdb_free((void *)call->batch_blob_columns[col]);
			}
		}
	}
	if (call->batch_out_varchar) {
		duckdb_free((void *)call->batch_out_varchar);
	}
	if (call->batch_out_blob) {
		duckdb_free((void *)call->batch_out_blob);
	}
	if (call->batch_out_list) {
		duckdb_free((void *)call->batch_out_list);
	}
	if (call->batch_out_array) {
		duckdb_free((void *)call->batch_out_array);
	}
	if (call->batch_out_struct) {
		duckdb_free((void *)call->batch_out_struct);
	}
	if (call->batch_out_map) {
		duckdb_free((void *)call->batch_out_map);
	}
	if (call->batch_out_union) {
		duckdb_free((void *)call->batch_out_union);
	}
	if (call->in_data) {
		duckdb_free(call->in_data);
	}
	if (call->in_validity) {
		duckdb_free(call->in_validity);
	}
	if (call->arg_ptrs) {
		duckdb_free(call->arg_ptrs);
	}
	if (call->batch_arg_data) {
		duckdb_free(call->batch_arg_data);
	}
	if (call->row_varchar_values) {
		duckdb_free((void *)call->row_varchar_values);
	}
	if (call->row_blob_values) {
		duckdb_free((void *)call->row_blob_values);
	}
	if (call->batch_varchar_columns) {
		duckdb_free((void *)call->batch_varchar_columns);
	}
	if (call->batch_blob_columns) {
		duckdb_free((void *)call->batch_blob_columns);
	}
	if (call->batch_varchar_owned) {
		duckdb_free((void *)call->batch_varchar_owned);
	}
	if (call->arg_value_bridges) {
		if (call->sig) {
			for (col = 0; col < call->sig->arg_count; col++) {
				if (call->arg_value_bridges[col]) {
					tcc_value_bridge_destroy(call->arg_value_bridges[col]);
				}
			}
		}
		duckdb_free((void *)call->arg_value_bridges);
	}
}
